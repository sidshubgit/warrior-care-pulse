...
create policy if not exists "care_team_participant_can_select_own"
  on public.care_team
  for select
  using (participant_id = auth.uid());

create policy if not exists "care_team_clinician_can_select_own"
  on public.care_team
  for select
...
create policy if not exists "check_ins_select_self_or_authorized_clinician"
  on public.check_ins
  for select
  using (
